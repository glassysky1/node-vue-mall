module.exports = app => {
  ...
  const Product = require('../../models/Product')
  //测试接口
  router1.use('/productlist', async (req, res) => {
    const products = await Product.find().populate('storages')
    // [
    //   {
     ...
     *       ipone  128,256  红，绿
     * 
     *      select distinct(type,color) from productinfo where name = ipone
     * 
     *      type  color
     *      128,256  红，绿
     * 
     ...
     * 
     *     ipone 128  红
     * select * from product where name=ipone and type =128 and color =‘红’
     * 
     * 
          */
